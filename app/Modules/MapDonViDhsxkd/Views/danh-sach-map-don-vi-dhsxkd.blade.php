<table id="order-listing" class="table table-hover table-striped">
    <thead>
        <tr class="background-vnpt text-center">
            <th>STT #</th>
            <th>Tên đơn vị</th>
            <th>Tên đơn vị ĐHSXKD</th>
            <th>ID đơn vị ĐHSXKD</th>
            <th>Trạng thái</th>
            <th>Xử lý</th>
        </tr>
    </thead>
    <tbody>                       
        ...
        <?php 
            $stt=0;
        ?>
        @foreach($danhSachMaps as $map)
            <?php $stt++; ?>
            <tr class="tr-hover tr-small">
                <td class="text-center">{{$stt}}</td>
                <td class='text-primary btn-sua' data="{{$map['id']}}">
                    {{$map['ten_don_vi']}}
                </td>
                <td>                    
                    {{$map['ten_don_vi_dhsxkd']}}
                </td>
                <td>                    
                    {{$map['id_don_vi_dhsxkd']}}
                </td>
                <td>
                    <label class=" @if($map['state']==1) {{'text-primary'}} @else {{'text-danger'}} @endif">@if($map['state']==1) {{'Đang hoạt động'}} @else {{'Ngừng hoạt động'}} @endif</label>
                </td>
                    ...
                    <button class="btn btn-vnpt" href="#" data-toggle="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right navbar-dropdown preview-list" aria-labelledby="notificationDropdown">
                            <a class="dropdown-item preview-item">
                                <p class="mb-0 font-weight-normal float-left text-primary btn-sua" data="{{$map['id']}}"><b><i class="icon-wrench"></i> Sửa</b>
                                </p>
                            </a>
                            <div class="dropdown-divider"></div>
                            <a class="dropdown-item preview-item">
                                <p class="mb-0 font-weight-normal float-left text-danger btn-xoa" data="{{$map['id']}}"><b><i class="icon-basket "></i> Xóa</b>
                                </p>
                            </a>                                 
                        </div>
                    </button>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
           <div class="modal-header background-vnpt">
              <h5 class="modal-title">SỬA MAP ĐƠN VỊ ĐHSXKD</h5>
...
<script type="text/javascript">
    jQuery(document).ready(function() {
        ...
        jQuery('.btn-sua').on('click',function(){            
            ...
            getById(_token, id, "{{ route('map-don-vi-dhsxkd-single') }}", ".frm-cap-nhat"); // gọi sự kiện lấy dữ liệu theo id
            $('#modal-cap-nhat').modal('show'); // bật form sửa     
        });

        /*Sự kiện bấm nút cập nhật*/
        jQuery('.btn-cap-nhat').on('click',function(){            
            capNhat(_token, $("form#frm-cap-nhat"), "{{ route('cap-nhat-map-don-vi-dhsxkd') }}", "{{ route('danh-sach-map-don-vi-dhsxkd') }}", '.load-danh-sach'); // bật form sửa     
        ...
        jQuery('.btn-xoa').on('click',function(){      
            ...
            if (result) {
                xoa(_token, id, "{{ route('xoa-map-don-vi-dhsxkd') }}", "{{ route('danh-sach-map-don-vi-dhsxkd') }}", '.load-danh-sach');  
            }
        });
