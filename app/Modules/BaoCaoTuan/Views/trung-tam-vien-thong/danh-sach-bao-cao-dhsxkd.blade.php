        ...
        <tr class="tr-hover tr-small">
          ...
          <td class="text-center">
            <form class="forms-sample frm-cap-nhat-ghi-chu" name="frm-cap-nhat-ghi-chu">
              {{ csrf_field() }}
              <input type="Text" class="form-control @if ($daChotSoLieu==1) disabled @endif ghi_chu" placeholder="" @if ($daChotSoLieu==1) disabled="disabled" @endif value="{{$pakn['ghi_chu']}}" name="ghi_chu">
...
<div class="row">
  <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
    <div class="form-group mt-5 text-right" style="margin-bottom: 0px;">
      <button type="button" class="btn btn-danger mr-2 btn-trung-tam-vien-thong-lay-so-lieu-bao-cao-dhsxkd @if ($daChotSoLieu==1) disabled @endif" @if ($daChotSoLieu==1) disabled="disabled" @endif><i class="fa fa-refresh"></i> Lấy dữ liệu</button>
    </div>
  </div>
</div>
...
<script type="text/javascript">
    jQuery(document).ready(function() {
      ...
      jQuery('.btn-trung-tam-vien-thong-lay-so-lieu-bao-cao-dhsxkd').on('click', function() {
        var _token=jQuery('form[name="frm-bao-cao-tuan"]').find("input[name='_token']").val();
        var idTuan=jQuery('#id_tuan').val(); 
        postAndRefreshById(_token, idTuan, "{{ route('trung-tam-vien-thong-lay-so-lieu-bao-cao-dhsxkd') }}", idTuan, "{{ route('trung-tam-vien-thong-danh-sach-bao-cao-dhsxkd') }}", '.load-danh-sach-dhsxkd',false);
      });
