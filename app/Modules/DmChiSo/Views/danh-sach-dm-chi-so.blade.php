<table id="order-listing" class="table table-hover">
    ...
    <tbody>                       
        ...
        <?php 
            $stt=0;
        ?>
        @foreach($dmChiSos as $dmChiSo)
            <?php $stt++; ?>
            <tr class="tr-hover tr-small">
                <td class="text-center">{{$stt}}</td>
                <td class='btn-sua' data="{{$dmChiSo['id']}}">
                    <div class="@if ($dmChiSo['parent_id']=='' || $dmChiSo['parent_id']==null) text-primary @endif ">{{$dmChiSo['chi_so']}}</div>
                </td>
                <td>
                    {{$dmChiSo['mo_ta']}}
                </td>
                <td>
                @php
                    $parent=Helper::getParentDmChiSo($dmChiSo['parent_id']);
                    if($parent){
                        echo $parent['mo_ta'] ? $parent['mo_ta']:$parent['chi_so'];
                    }
                    ...
                    <button class="btn btn-vnpt" href="#" data-toggle="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right navbar-dropdown preview-list" aria-labelledby="notificationDropdown">
                            <a class="dropdown-item preview-item">
                                <p class="mb-0 font-weight-normal float-left text-primary btn-sua" data="{{$dmChiSo['id']}}"><b><i class="icon-wrench"></i> Sửa</b>
                                </p>
                            </a>
                            <div class="dropdown-divider"></div>
                            <a class="dropdown-item preview-item">
                                <p class="mb-0 font-weight-normal float-left text-danger btn-xoa" data="{{$dmChiSo['id']}}"><b><i class="icon-basket "></i> Xóa</b>
                                </p>
                            </a>                                 
                        </div>
                    </button>
...
<script type="text/javascript">
    jQuery(document).ready(function() {
        ...
        jQuery('.btn-sua').on('click',function(){            
            ...
            getById(_token, id, "{{ route('dm-chi-so-single') }}", ".frm-cap-nhat"); // gọi sự kiện lấy dữ liệu theo id
            $('#modal-cap-nhat').modal('show'); // bật form sửa     
        });

        /*Sự kiện bấm nút cập nhật*/
        jQuery('.btn-cap-nhat').on('click',function(){            
            capNhat(_token, $("form#frm-cap-nhat"), "{{ route('cap-nhat-dm-chi-so') }}", "{{ route('danh-sach-dm-chi-so') }}", '.load-danh-sach'); // bật form sửa     
        ...
        jQuery('.btn-xoa').on('click',function(){      
            ...
            if (result) {
                xoa(_token, id, "{{ route('xoa-dm-chi-so') }}", "{{ route('danh-sach-dm-chi-so') }}", '.load-danh-sach');  
            }
        });
