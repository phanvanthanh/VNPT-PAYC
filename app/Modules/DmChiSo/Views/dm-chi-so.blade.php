...
@section('content')
	<div class="col-12">
        <div class="card">
            <div class="card-body">
                <div class="row">
                  <div class="col-6">
                    <h4 class="text-danger">DANH MỤC CHỈ SỐ ĐHSXKD</h4>
                  </div>
                    <div class="col-6">
                ...
                <div class="text-right table-responsive">
                    <div class="btn-group mr-2">
                        <button class="btn btn-sm btn-vnpt btn-load-form-them-moi" data-toggle="modal" data-target="#modal-them-moi"><i class="mdi mdi-plus-circle-outline"></i> Thêm chỉ số</button>
                    </div>
                </div>
                <br>
         ...
         <div class="modal-dialog" role="document">
            <div class="modal-content">
               <div class="modal-header background-vnpt">
                  <h5 class="modal-title">THÊM MỚI CHỈ SỐ ĐHSXKD</h5>
  ...
  <script type="text/javascript">
    jQuery(document).ready(function() {
      ...
      loadTable(_token, "{{ route('danh-sach-dm-chi-so') }}", '.load-danh-sach');

      $('.btn-them-moi').on('click',function(){
          themMoi(_token, $("form#frm-them-moi"), "{{ route('them-dm-chi-so') }}", "{{ route('danh-sach-dm-chi-so') }}", '.load-danh-sach');
          jQuery("#modal-them-moi").modal('hide');
      });

      $('.btn-load-form-them-moi').on('click',function(){
        getById(_token, "", "{{ route('dm-chi-so-single') }}", ".frm-them-moi"); // gọi sự kiện lấy dữ liệu theo id
      });
