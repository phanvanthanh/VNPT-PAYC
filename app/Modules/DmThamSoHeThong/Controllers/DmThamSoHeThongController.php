...
class DmThamSoHeThongController extends Controller{
    /**
    ...
    public function dmThamSoHeThong(Request $request){
        return view('DmThamSoHeThong::dm-tham-so-he-thong');
    }


    public function danhSachDmThamSoHeThong(Request $request){
        if(RequestAjax::ajax()){ // Kiểm tra gửi đường ajax
            $error=''; // Khai báo biến
            $dmThamSoHeThongs=DmThamSoHeThong::all()->toArray();
            $view=view('DmThamSoHeThong::danh-sach-dm-tham-so-he-thong', compact('dmThamSoHeThongs','error'))->render(); // Trả dữ liệu ra view 
    ...
    public function themDmThamSoHeThong(Request $request){
        if(RequestAjax::ajax()){ // Kiểm tra gửi đường ajax
            $data=RequestAjax::all(); // Lấy tất cả dữ liệu
            DmThamSoHeThong::create($data); // Lưu dữ liệu vào DB
    ...
    public function dmThamSoHeThongSingle(Request $request){
        if(RequestAjax::ajax()){ // Kiểm tra gửi đường ajax
            ...
            if(isset($dataForm['id'])){ // ngược lại dữ liệu hợp lệ
                $data = DmThamSoHeThong::where("id","=",$dataForm['id'])->get(); // kiểm tra dữ liệu trong DB
            ...
            $view=view('DmThamSoHeThong::dm-tham-so-he-thong-single', compact('data','error'))->render(); // Trả dữ liệu ra view trước     
    ...
    public function capNhatDmThamSoHeThong(Request $request){
        if(RequestAjax::ajax()){ // Kiểm tra phương thức gửi dữ liệu là AJAX
            ...
            $dmThamSoHeThong=DmThamSoHeThong::where("id","=",$id)->get()->toArray();
            if(count($dmThamSoHeThong)==1){
                unset($dataForm["_token"]);
                $dmThamSoHeThong=DmThamSoHeThong::where("id","=",$id);
                $dmThamSoHeThong->update($dataForm);
                return array("error"=>'');
            }else{
                return array("error"=>'Không tìm thấy dữ liệu cần sửa'); // Trả lỗi về AJAX
            }         
    ...
    public function xoaDmThamSoHeThong(Request $request){
        if(RequestAjax::ajax()){ // Kiểm tra phương thức gửi dữ liệu là AJAX
            $dataForm=RequestAjax::all(); // Lấy tất cả dữ liệu đã gửi
            ...
            $id=$dataForm['id']; //ngược lại có id
            $dmThamSoHeThong=DmThamSoHeThong::where("id","=",$id)->get();
            if(count($dmThamSoHeThong)==1){
                DmThamSoHeThong::where("id","=",$id)->delete();
                return array("error"=>'');
            }else{
                return array("error"=>'Không tìm thấy dữ liệu cần xóa'); // Trả lỗi về AJAX
            }         
