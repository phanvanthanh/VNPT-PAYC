...
class NhomChucVuController extends Controller{
    /**
    ...
    public function nhomChucVu(Request $request){
        return view('NhomChucVu::nhom-chuc-vu');
    }


    public function danhSachNhomChucVu(Request $request){
        if(RequestAjax::ajax()){ // Kiểm tra gửi đường ajax
            $error=''; // Khai báo biến
            $nhomChucVus=NhomChucVu::all()->toArray();
            $view=view('NhomChucVu::danh-sach-nhom-chuc-vu', compact('nhomChucVus','error'))->render(); // Trả dữ liệu ra view 
    ...
    public function themNhomChucVu(Request $request){
        if(RequestAjax::ajax()){ // Kiểm tra gửi đường ajax
            $data=RequestAjax::all(); // Lấy tất cả dữ liệu
            NhomChucVu::create($data); // Lưu dữ liệu vào DB
    ...
    public function nhomChucVuSingle(Request $request){
        if(RequestAjax::ajax()){ // Kiểm tra gửi đường ajax
            ...
            if(isset($dataForm['id'])){ // ngược lại dữ liệu hợp lệ
                $data = NhomChucVu::where("id","=",$dataForm['id'])->get(); // kiểm tra dữ liệu trong DB
            ...
            $view=view('NhomChucVu::nhom-chuc-vu-single', compact('data','error'))->render(); // Trả dữ liệu ra view trước     
    ...
    public function capNhatNhomChucVu(Request $request){
        if(RequestAjax::ajax()){ // Kiểm tra phương thức gửi dữ liệu là AJAX
            ...
            $nhomChucVus=NhomChucVu::where("id","=",$id)->get()->toArray();
            if(count($nhomChucVus)==1){
                unset($dataForm["_token"]);
                $nhomChucVus=NhomChucVu::where("id","=",$id);
                $nhomChucVus->update($dataForm);
                return array("error"=>'');
            }else{
                return array("error"=>'Không tìm thấy dữ liệu cần sửa'); // Trả lỗi về AJAX
            }         
    ...
    public function xoaNhomChucVu(Request $request){
        if(RequestAjax::ajax()){ // Kiểm tra phương thức gửi dữ liệu là AJAX
            $dataForm=RequestAjax::all(); // Lấy tất cả dữ liệu đã gửi
            ...
            $id=$dataForm['id']; //ngược lại có id
            $nhomChucVu=NhomChucVu::where("id","=",$id)->get();
            if(count($nhomChucVu)==1){
                NhomChucVu::where("id","=",$id)->delete();
                return array("error"=>'');
            }else{
                return array("error"=>'Không tìm thấy dữ liệu cần xóa'); // Trả lỗi về AJAX
            }         
