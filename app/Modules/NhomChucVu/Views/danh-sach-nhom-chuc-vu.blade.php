<table id="order-listing" class="table table-hover">
    <thead>
        <tr class="background-vnpt text-center">
            <th>STT #</th>
            <th>Tên nhóm chức vụ</th>
            <th>Mã nhóm chức vụ</th>
            <th>Trạng thái</th>
            <th>Xử lý</th>
        </tr>
    </thead>
    <tbody>                       
        ...
        @foreach($nhomChucVus as $nhomChucVu)
            ...
            <tr class="tr-hover tr-small">
                ...
                <td class='text-primary btn-sua' data="{{$nhomChucVu['id']}}">
                    {{$nhomChucVu['ten_nhom_chuc_vu']}}
                </td>
                <td>                    
                    {{$nhomChucVu['ma_nhom_chuc_vu']}}
                </td>
                <td>
                    <label class=" @if($nhomChucVu['state']==1) {{'text-primary'}} @else {{'text-danger'}} @endif">@if($nhomChucVu['state']==1) {{'Đang hoạt động'}} @else {{'Ngừng hoạt động'}} @endif</label>
                </td>
                    ...
                    <button class="btn btn-vnpt" href="#" data-toggle="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right navbar-dropdown preview-list" aria-labelledby="notificationDropdown">
                            <a class="dropdown-item preview-item">
                                <p class="mb-0 font-weight-normal float-left text-primary btn-sua" data="{{$nhomChucVu['id']}}"><b><i class="icon-wrench"></i> Sửa</b>
                                </p>
                            </a>
                            <div class="dropdown-divider"></div>
                            <a class="dropdown-item preview-item">
                                <p class="mb-0 font-weight-normal float-left text-danger btn-xoa" data="{{$nhomChucVu['id']}}"><b><i class="icon-basket "></i> Xóa</b>
                                </p>
                            </a>                                 
                        </div>
                    </button>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
           <div class="modal-header background-vnpt">
              <h5 class="modal-title">SỬA NHÓM CHỨC VỤ</h5>
...
<script type="text/javascript">
    jQuery(document).ready(function() {
        ...
        jQuery('.btn-sua').on('click',function(){            
            ...
            getById(_token, id, "{{ route('nhom-chuc-vu-single') }}", ".frm-cap-nhat"); // gọi sự kiện lấy dữ liệu theo id
            $('#modal-cap-nhat').modal('show'); // bật form sửa     
        });

        /*Sự kiện bấm nút cập nhật*/
        jQuery('.btn-cap-nhat').on('click',function(){            
            capNhat(_token, $("form#frm-cap-nhat"), "{{ route('cap-nhat-nhom-chuc-vu') }}", "{{ route('danh-sach-nhom-chuc-vu') }}", '.load-danh-sach'); // bật form sửa     
        ...
        jQuery('.btn-xoa').on('click',function(){      
            ...
            if (result) {
                xoa(_token, id, "{{ route('xoa-nhom-chuc-vu') }}", "{{ route('danh-sach-nhom-chuc-vu') }}", '.load-danh-sach');  
            }
        });
