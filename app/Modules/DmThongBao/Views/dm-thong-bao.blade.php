...
@section('content')
	<div class="col-12">
        <div class="card">
            <div class="card-body">
                <div class="row">
                  <div class="col-6">
                    <h4 class="text-danger">DANH MỤC THÔNG BÁO HỆ THỐNG</h4>
                  </div>
                    <div class="col-6">
  ...
  <script type="text/javascript">
    jQuery(document).ready(function() {
      ...
      loadTable(_token, "{{ route('danh-sach-dm-thong-bao') }}", '.load-danh-sach');

      $('.btn-them-moi').on('click',function(){
          themMoi(_token, $("form#frm-them-moi"), "{{ route('them-dm-thong-bao') }}", "{{ route('danh-sach-dm-thong-bao') }}", '.load-danh-sach');
          jQuery("#modal-them-moi").modal('hide');
      });

      $('.btn-load-form-them-moi').on('click',function(){
        getById(_token, "", "{{ route('dm-thong-bao-single') }}", ".frm-them-moi"); // gọi sự kiện lấy dữ liệu theo id
      });
