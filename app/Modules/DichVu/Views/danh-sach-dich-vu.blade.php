...
<script type="text/javascript">
    jQuery(document).ready(function() {
        ...
        jQuery('.btn-sua').on('click',function(){            
            ...
            getById(_token, id, "{{ route('dich-vu-single') }}", ".frm-cap-nhat"); // gọi sự kiện lấy dữ liệu theo id
            $('#modal-cap-nhat').modal('show'); // bật form sửa     
        });

        /*Sự kiện bấm nút cập nhật*/
        jQuery('.btn-cap-nhat').on('click',function(){            
            capNhat(_token, $("form#frm-cap-nhat"), "{{ route('cap-nhat-dich-vu') }}", "{{ route('danh-sach-dich-vu') }}", '.load-danh-sach'); // bật form sửa     
        ...
        jQuery('.btn-xoa').on('click',function(){      
            ...
            if (result) {
                xoa(_token, id, "{{ route('xoa-dich-vu') }}", "{{ route('danh-sach-dich-vu') }}", '.load-danh-sach');  
            }
        });
