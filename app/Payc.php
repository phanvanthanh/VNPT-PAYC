...
class Payc extends Authenticatable
{
    ...
    protected $fillable = [
        'id','id_user_tao', 'id_dich_vu', 'tieu_de', 'noi_dung', 'file_payc', 'ngay_tao', 'han_xu_ly_mong_muon', 'han_xu_ly_duoc_giao', 'ngay_hoan_tat', 'trang_thai', 'so_phieu', 'ma_phuong_xa', 'vi_do', 'kinh_do', 'loai_cong_viec', 'sap_xep'
    ];
    ...
    public static function getDanhSachPaycChoTiepNhan($userId){
        ...
        $data=DB::select('SELECT cbxl.id, p.id_user_tao, p.tieu_de, p.noi_dung, p.file_payc, p.ngay_tao, p.han_xu_ly_mong_muon, p.han_xu_ly_duoc_giao, p.ngay_hoan_tat, ttxl.ma_trang_thai,
            ...
            and (ttxl.ma_trang_thai="TAO_MOI" or ttxl.ma_trang_thai="CHUYEN_DON_VI_CAP_TREN" or ttxl.ma_trang_thai="CAP_NHAT")
            and cbn.id_user_nhan='.$userId.'
            order by p.id desc');
        //   or ttxl.ma_trang_thai="CHUYEN_CAN_BO"
        $data = collect($data)->map(function($x){ return (array) $x; })->toArray(); 
        return $data;
    ...
    public static function getDanhSachPaycChoXuLy($userId){
        $data=array();
            ...
            and ((cbn.vai_tro=0 AND cbn.trang_thai=0) OR (cbn.vai_tro=1 AND (cbn.trang_thai=0 OR cbn.trang_thai=1)) OR (cbn.vai_tro=2 AND (cbn.trang_thai=0 OR cbn.trang_thai=1)))
            and cbn.id_user_nhan='.$userId.'
            order by p.id desc');
        $data = collect($data)->map(function($x){ return (array) $x; })->toArray(); 
        return $data;
