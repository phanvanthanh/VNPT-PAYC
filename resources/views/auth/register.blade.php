				...
				<div class="row">
					...
					<div class="col-6 text-center">
						...
						<form class="forms-sample" method="POST" action="{{ route('register') }}" enctype="multipart/form-data">
							...
							<div class="form-group row">
							  	<label for="ten-dang-nhap" class="col-sm-3 col-form-label">Tên đăng nhập <b class="text-danger">(*)</b></label>
							  	<div class="col-sm-9 text-left">
							    	<input type="Text" class="form-control {{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" placeholder="Nhập tên đăng nhập" value="{{ old('email')}}" required="required">
							    	@if ($errors->has('email'))
							            <span class="invalid-feedback" role="alert">
							...
							<button type="submit" class="btn btn-vnpt mr-2 "> <i class="icon-check"></i> Đăng ký tài khoản</button>
							<a href= "{{ route('login') }}" class="btn btn-danger">Quay lại</a>
						</form>
					</div>
				</div>
